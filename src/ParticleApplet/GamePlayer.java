...
class Particle {
	...
	//Method that returns the index x
	public int getX() {
		// TODO Auto-generated method stub
		return x;
	}
	//Method that returns the index y
	public int getY() {
		// TODO Auto-generated method stub
		return y;
	}
...
@SuppressWarnings("serial")
//Change 2: replaced extends Applet with Frame
class AnimationApplet extends Frame { // Fragments
	// ...
	...
	//Change 3: added a getParameter(String) method to return the FPS 
...
class CellUsingReorderedBackoff {
	...
	private static boolean trySwap(CellUsingReorderedBackoff a, CellUsingReorderedBackoff b)
...
class Subject {
	...
	protected final EDU.oswego.cs.dl.util.concurrent.CopyOnWriteArrayList observers = new EDU.oswego.cs.dl.util.concurrent.CopyOnWriteArrayList();
...
class TransactionLogger {
	void cancelLogEntry(Transaction t, long amount, TransBankAccount src, TransBankAccount dst) {
	}

	void logTransfer(Transaction t, long amount, TransBankAccount src, TransBankAccount dst) {
	}

	void logCompletedTransfer(Transaction t, long amount, TransBankAccount src, TransBankAccount dst) {
	}
...
class AccountUser {
	...
	public boolean transfer(long amount, TransBankAccount src, TransBankAccount dst)
...
class FibVCB extends FJTask {
	...
	public void run() { // same structure as join-based version
		...
		if (n <= sequentialThreshold)
			...
			number = 0;
			// establish number of call backs expected
			callbacksExpected = 2;
			...
			/*
			 * Making the current thread wait for the children to finish their tasks
			 */
			// Wait for call backs from children
...
class Jacobi extends FJTask {
	...
	Jacobi(double[][] A, double[][] B, int firstRow, int lastRow, int firstCol, int lastCol, int maxSteps,
			int leafCells) {
...
class College implements jcsp.lang.CSProcess {
	...
	College() {
		...
		action = new jcsp.lang.Parallel(
				new jcsp.lang.CSProcess[] { butler, new jcsp.lang.Parallel(phils), new jcsp.lang.Parallel(forks) });
	}
